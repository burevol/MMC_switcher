...
Процедура ОтобразитьКнопки(Группа, СписокПлатформ)
    Индекс = 0;
...
Функция СоздатьГруппуКнопок(ФормаПриложения, КоличествоКнопок)
...
ГруппаКнопок = СоздатьГруппуКнопок(Форма1, КоличествоКНопок);
ОтобразитьКнопки(ГруппаКнопок, СписокПлатформ);
