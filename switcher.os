...
Функция ПолучитьСоответствиеПлатформ()
    ...
    Для Каждого УстановленнаяВерсия Из СписокУстановленныхПлатформ Цикл
        ПутьКПредприятию = Платформа1С.ПутьКПредприятию(
            УстановленнаяВерсия.НомерВерсии, УстановленнаяВерсия.Разрядность
            );
...
Процедура ПриНажатииКнопки() Экспорт
    ИмяФайла = Ф.АргументыСобытия.Параметр + "radmin.dll";
    Если Новый Файл(ИмяФайла).Существует() Тогда
        Команда = Новый Команда;
        Команда.УстановитьСтрокуЗапуска(
            "powershell Start ""regsvr32.exe"" -Verb RunAs -Argument '""""""" + ИмяФайла + """""""' "
            );
        КодВозврата = Команда.Исполнить();
        Сообщить(КодВозврата);
        Сообщить(Команда.ПолучитьВывод());
    Иначе
        Ф.ОкноСообщений().Показать(
            "Не найдена библиотека управления сервером", 
            "Ошибка", 
            Ф.КнопкиОкнаСообщений.ОК, 
            Ф.ЗначокОкнаСообщений.Восклицание
            );
    КонецЕсли;
КонецПроцедуры

Процедура ПриНажатииКнопкиЗапустить() Экспорт
    ЗапуститьПриложение("mmc ""C:\Program Files\1cv8\common\1CV8 Servers (x86-64).msc""");
КонецПроцедуры
...
Процедура ОтобразитьКнопки(Группа)
    ...
    Для Каждого Платформа Из СписокПлатформ Цикл
        ...
        Кнопка.Границы = Ф.Прямоугольник(
            ШИРИНА_ОТСТУПА, ВЫСОТА_КНОПКИ * Индекс + ВЫСОТА_ОТСТУПА, ШИРИНА_КНОПКИ, ВЫСОТА_КНОПКИ
            );
    ...
    Кнопка.Текст = "Запустить";
    Кнопка.Нажатие = Ф.Действие(ЭтотОбъект, "ПриНажатииКнопкиЗапустить");
КонецПроцедуры
...
Функция СоздатьГруппуКнопок(ФормаПриложения)
    ...
    ГруппаКнопок.Границы = Ф.Прямоугольник(
        ...
        (КоличествоКНопок + 1) * ВЫСОТА_КНОПКИ +  (КоличествоКНопок + 1) * ВЫСОТА_ОТСТУПА);
